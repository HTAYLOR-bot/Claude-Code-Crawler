...
def _allowed_product_suffix_token(token: str) -> bool:
    ...
    if re.fullmatch(r"(?:xl|tl|rf|zr|ev|oe|runflat|run-flat)", t, re.I):
        return True
    if re.fullmatch(r"[a-z]{0,4}\d[a-z0-9-]*", t, re.I):
        return True
    return False
...
def product_name_matches_strict(text: Optional[str], product_name: Optional[str]) -> bool:
    ...
    for m in pattern.finditer(hay):
        ...
        if not after:
            return True
        next_token = re.split(r"\s+", after, maxsplit=1)[0]
        if _allowed_product_suffix_token(next_token):
            return True
    return False
...
class GoogleShoppingCrawler:
    ...
    def launch(self) -> List[Dict[str, Any]]:
        ...
        with sync_playwright() as pw:
            ...
            while True:
                browser = pw.chromium.launch(headless=headless, slow_mo=0 if headless else 80)
                ...
                try:
                    ...
                    # open_search() already handles verification immediately after the first goto.
                    # Avoid a second verification prompt/log loop on the same landing page.
